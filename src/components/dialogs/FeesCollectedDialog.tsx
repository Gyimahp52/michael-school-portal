...
interface FeesCollectedDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function FeesCollectedDialog({ open, onOpenChange }: FeesCollectedDialogProps) {
  ...
  const loadMonthlyInvoices = async () => {
    try {
      ...
      const monthlyPaidInvoices = invoicesData
      ...
      setInvoices(monthlyPaidInvoices);
    } catch (error) {
      console.error('Error loading monthly invoices:', error);
    } finally {
      setLoading(false);
    }
  };

  const totalFeesCollected = invoices.reduce((sum, inv) => sum + (inv.amount || 0), 0);
          ...
          <div className="flex items-center justify-between p-4 bg-success/10 rounded-lg border border-success/20">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(totalFeesCollected)}</p>
              </div>
            </div>
